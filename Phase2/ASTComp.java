public class ASTComp implements ASTNode {
    ...
    public IValue eval(Environment<IValue> env) throws InterpreterError{
        ...
        ASTType type1 = t1.typecheck(env);
        ASTType type2 = t2.typecheck(env);

        // == and ~= work on values of the same type
        if ((op == "==") || (op == "~=")) {
            if (!type1.subtypeOf(type2,env))
                throw new TypeCheckError("Operands must have the same type");
            return new ASTTBool();
        }

        if (!(type1 instanceof ASTTInt) || !(type2 instanceof ASTTInt))
            throw new TypeCheckError("Operands must be type int");
        return new ASTTBool();
    }
}